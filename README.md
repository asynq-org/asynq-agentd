...
For dashboard-style Buddy clients, the daemon now also exposes `GET /dashboard/overview`, `GET /dashboard/attention-required`, and `GET /dashboard/continue-working`, so the UI can render high-level cards without stitching together raw session/task/approval endpoints client-side.
