...
- Recent-work indexing and continuation from `~/.claude/`, `~/.codex/`, and Cursor `User` storage
...
Codex-backed tasks currently expect a working `codex` CLI on `PATH` (or `ASYNQ_AGENTD_CODEX_BIN` pointing to it). Claude-backed tasks expect a working `claude` CLI on `PATH` or at `~/.local/bin/claude` (or `ASYNQ_AGENTD_CLAUDE_BIN` pointing to it), plus an authenticated Claude Code session. Cursor-backed tasks expect a working `cursor-agent` CLI on `PATH` (or `ASYNQ_AGENTD_CURSOR_BIN` pointing to it), plus authenticated Cursor CLI access.
